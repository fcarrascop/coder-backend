...
class CartManager {
    ...
    async getObjectId(id) {
        ...
        if (!result || Object.keys(result).length === 0) return ({"error": "Producto no encontrado", id: id})
    ...
    async getCartId(_id) {
        ...
        if (!result || Object.keys(result).length === 0) return ({error: "No se ha encontrado el carrito"})
        return (result.id)
    }
    ...
    async createCart(products) {
        ...
        if (products?.products) {
            for (const item of products.products) {
                ...
                if (objectId?.error) return ({error: "Uno o más productos no encontrados", item: item.id})
    ...
    async addProducts(id, idProduct) {
        ...
        if (!cart || Object.keys(cart).length === 0) return ({error: "Carrito no encontrado"})
    ...
    async updateProductQuantity(idCart, idProduct, quantity = 0) {
        await this.Update()
